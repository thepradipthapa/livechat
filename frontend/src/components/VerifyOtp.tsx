...
import { backend_service, useAppData } from '@/context/AppContext';
...
const VerifyOtp = () => {
    ...
    const handelSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        setLoading(true);
        try {
            const {data} = await axios.post(`${backend_service}/api/v1/accounts/verify/`, { email, otp: otpValue });
    ...
    const handleResendOTP = async () => {
        ...
        setError('');
        try {
            const { data } = await axios.post(`${backend_service}/api/v1/accounts/login/`, { email });
            toast.success(data.message);
            setTimer(60);
        } catch (err: any) {
